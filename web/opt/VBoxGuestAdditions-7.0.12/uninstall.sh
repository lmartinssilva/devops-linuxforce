...
# Read routines.sh
if ! test -r "/opt/VBoxGuestAdditions-7.0.12/routines.sh"; then
    echo 1>&2 "Required file routines.sh not found.  Aborting..."
    return 1
fi
. "/opt/VBoxGuestAdditions-7.0.12/routines.sh"
...
echo 1>&2 "Removing installed version 7.0.12 of VirtualBox Guest Additions..."
...
if test -r "/opt/VBoxGuestAdditions-7.0.12"/init/vboxadd-service; then
...
if test -r "/opt/VBoxGuestAdditions-7.0.12"/init/vboxadd; then
    stop_init_script vboxadd 2>> "$LOGFILE"
    test -n "$NO_CLEANUP" || "/opt/VBoxGuestAdditions-7.0.12"/init/vboxadd cleanup 2>> "$LOGFILE"
...
for CUR_MODULE in /opt/VBoxGuestAdditions-7.0.12/installer/module-autologon
    do
        . "$CUR_MODULE"
    done
